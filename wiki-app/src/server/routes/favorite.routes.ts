...
import { favorites, pages as pagesTable, branches } from "../db/schema.js";
...
export async function favoriteRoutes(app: FastifyInstance) {
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      const rows = await db
        ...
        .innerJoin(pagesTable, eq(pagesTable.id, branches.pageId))
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      await db.insert(favorites).values({ userId: user.id, branchId } as any);
