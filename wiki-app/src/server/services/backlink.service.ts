...
export async function getPageBacklinks(pageId: string): Promise<BacklinkEntry[]> {
  ...
  const rows = await db
    ...
    .where(and(
      // drizzle-better-sqlite3 doesn't have an inArray shorthand for multi-columns;
      // scan for each placement and merge.
      eq(backlinks.targetBranchId, branchIds[0]!)
    ));

  // Simplification: only handle the first placement. Multi-placement backlinks
  // are rare and this keeps the query straightforward.
  const mergedRows = branchIds.length > 1
    ? [
        ...rows,
        ...((await Promise.all(
          branchIds.slice(1).map((bid) =>
            db
              .select({
                sourcePageId: backlinks.sourcePageId,
                targetBlockId: backlinks.targetBlockId,
              ...
              .from(backlinks)
              .innerJoin(pages, eq(pages.id, backlinks.sourcePageId))
              .where(eq(backlinks.targetBranchId, bid))
          )
        )).flat()),
      ]
    : rows;

  // Dedupe by (sourcePageId, targetBlockId) — Drizzle returns arrays from
  // multiple queries, so we might get duplicates when a page is placed in
  // multiple branches.
  ...
  for (const r of mergedRows) {
    ...
    // Find a placement for the source page.
